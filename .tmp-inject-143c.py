#!/usr/bin/env python3
"""一次性：143c（签名第一测 cookie_dump + net_like extra_headers）打包 TikTok_XNOW_v1.4.143c_BH.ipa"""
...
LOCAL_DYLIB = os.path.join(PROJECT, 'build-artifacts-ci', 'xnower-143c', 'xnower.dylib')
OUT = 'TikTok_XNOW_v1.4.143c_BH.ipa'
...
print("[1] 上传 xnower.dylib → /root/xnow-build/xnower-143c.dylib ...")
sftp = ssh.open_sftp()
sftp.put(LOCAL_DYLIB, '/root/xnow-build/xnower-143c.dylib')
sftp.close()
print("  uploaded")
...
run(f"cd /opt/xnow-flow && python3 vps-inject.py {base_ipa} /root/xnow-build/xnower-143c.dylib /opt/xnow-flow/static/{OUT} 2>&1", 300)
