"""build-vps-dylib-131.py — VPS 交叉编译 xnower.dylib（v1.4.131）
v1.4.131 = 触摸盲区根治：IOHIDEvent 真实事件注入
  根因: KVC 合成 UITouch 的 _gestureRecognizers 为空 → 手势识别器收不到事件 → TikTok 全按钮假成功
  方案: IOHIDEvent digitizer 事件经 _handleHIDEvent: 注入（iOS 触摸系统真入口，手势状态机完整跑）
  解锁: like / follow / search / 头像 / follow_user / open_tab acc_id_tap / go_home profile
  降级: 符号/ContextID 取不到时回退旧 KVC 合成触摸
"""
