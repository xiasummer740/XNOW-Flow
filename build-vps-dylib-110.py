#!/usr/bin/env python3
"""build-vps-dylib-110.py — VPS 交叉编译 xnower.dylib（v1.4.110）
v1.4.110 = v1.4.109 + 浮窗弹窗修复：
  ⑧ 账号管理页「新增账号/备份当前账号」点了没反应（祥哥 108 装机实证）：
     根因 = overlay 窗口无 rootViewController，_presentAlert 找 ws.keyWindow 点到浮窗后
            keyWindow 可能是 overlay 且 rootVC 为 nil → present 静默失败
     → XNOWER.m overlay 挂 host rootVC + 面板挂 hostVC.view + XNPassThroughWindow hitTest
        跳过透明 hostVC.view 保穿透；XNFloatingPanel.m _presentAlert 改从 self.window.rootViewController present
...
# 6. Download
print("[6] Download...")
local_dir = os.path.join(PROJECT, 'build-artifacts-ci', 'xnower-110')
