"""build-vps-dylib-143.py — VPS 交叉编译 xnower.dylib（v1.4.143）
v1.4.143 = 纯网络层点赞第一波（祥哥方向 1→3→2）：
  ① net_like 改造：真实 host（api16-normal-useast5.tiktokv.us SNI 实锤）+
     会话 Cookie（NSHTTPCookieStorage .tiktok 域，含 install_id/msToken/odin_tt）+ UA 伪装；
     附 cookie_count/feed_headers_captured 诊断。
  ② TLS 栈探针（方向3）：SocketHooks tlsProbe —— dlsym 查 SSL_* 符号是否在动态符号表，
     定 BoringSSL 静态链接事实（SSL 钩子 0 命中根因）。
  脚本沿 142：上传/编译支持 .c 文件（fishhook.c）。
"""
