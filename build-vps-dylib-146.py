"""build-vps-dylib-145.py — VPS 交叉编译 xnower.dylib（v1.4.146）
v1.4.146 = follow UIAction 修复（2026-09-03 装机验证暴露）：
  XNTouchSimulator tapView 手势 invoke 补 iOS14+ UIAction _handlers 触发——
  FollowPromptView 关注按钮（feed 纯手势）用 addAction:forState: 注册，target 存
  _handlers 非 _targets；原代码只遍历 _targets → invoke 漏触发 → 关注点不动。
  加 _gestureInfo _handlers 诊断（装机后确认手势接线）。
  继承 144：导航 bug 修复（profile→home setSelectedIndex 优先）+ 构建炸弹清除
  （import 修复 + 清 VPS 目录）+ 143c 遗产（cookie_dump + net_like extra_headers）。
"""
...
# 0. Clean remote build dir（防残留兜底，144 构建炸弹修复延续）
