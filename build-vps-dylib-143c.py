...
v1.4.143c = 签名复刻第一测（后端 douyin-sign 硬套 iOS 43.7.0 digg 实验）：
  ① 新命令 cookie_dump：回传 TikTok 会话 Cookie 串 + install_id/msToken 值（签名实验取真实设备凭据）。
  ② net_like 支持 params.extra_headers：合并后端预生成 X-Argus 家族签名头（X-Khronos/X-SS-STUB/
     X-Gorgon/X-Argus），extra 优先覆盖（含 Cookie 用 extra 的保证签签一致）；附 sign_headers_used 诊断。
  前向 143b（端点 /aweme/v1/commit/item/digg/ + type=1）。
"""
