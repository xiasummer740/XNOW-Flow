#!/usr/bin/env python3
"""build-vps-dylib-109.py — VPS 交叉编译 xnower.dylib（v1.4.109）
v1.4.109 = v1.4.108 攒批（F14/F16/F21/F26/F6/B10/F13/F27/B41 + secret header）+ F6 捕获根因修复：
  ⑦ F6 保存视频抓不到 URL：装机实证 save_video 秒回(duration:0) = gLastFeedVideo 空，
     根因 = canInitWithRequest 只拦 tiktok.com/byteoversea.com，TikTok iOS 实际 feed 请求走 tiktokv.com 被排除
     → 按 /feed /recommend 路径级补拦 tiktokv.com（非 v1.4.39 的全量拦截，路径级+piggyback 限频，风险可控）
...
# 6. Download
print("[6] Download...")
local_dir = os.path.join(PROJECT, 'build-artifacts-ci', 'xnower-109')
