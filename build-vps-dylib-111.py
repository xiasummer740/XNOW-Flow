#!/usr/bin/env python3
"""build-vps-dylib-111.py — VPS 交叉编译 xnower.dylib（v1.4.111）
v1.4.111 = v1.4.110 + 浮窗标题带版本号：
  ⑨ 快捷菜单标题旁没显示当前版本号（祥哥反馈）：_showMainMenu 把 init 时带的 "XNOW v%@"
     覆盖成纯 "快捷菜单" → 改 "快捷菜单 v%@"，方便确认装的哪个版本
沿用 v1.4.110：⑧ 账号管理页弹窗修复（overlay 挂 host rootVC + _presentAlert 从 self.window present）。
...
# 6. Download
print("[6] Download...")
local_dir = os.path.join(PROJECT, 'build-artifacts-ci', 'xnower-111')
