#!/usr/bin/env python3
"""build-vps-dylib-106.py — VPS 交叉编译 xnower.dylib（v1.4.106）
v1.4.106 修复：open_tab home 切不回 feed。
根因：TikTok 首页 tab 的 setSelectedIndex:0 实测不落位（friends→home 卡在原 tab，vc_chain 证实
  AWEFeedRootViewController 类名正确但回读 selectedIndex 仍非 0）。go_home 深链 snssdk1233://feed 可靠回 feed。
修复：_tapTab: home 落位验证 + 深链兜底 + 1.2s 异步复验防 TikTok 回退；_tapTab 返回诊断 dict 随 result 回传。
...
# 6. Download
print("[6] Download...")
local_dir = os.path.join(PROJECT, 'build-artifacts-ci', 'xnower-106')
