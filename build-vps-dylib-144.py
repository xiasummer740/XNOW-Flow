"""build-vps-dylib-144.py — VPS 交叉编译 xnower.dylib（v1.4.144）
v1.4.144 = like/follow 导航 bug 修复批（2026-09-03 攒批）：
  ① CommandActionGoHome 用 _gotoHomeFeed 返回值——4 轮耗尽标 failed，不再 28s 假成功。
  ② _tapTab:home 非 feed 状态先走 setSelectedIndex:0（profile tab 页稳定路径，
     129 短路逻辑只 tap+深链导致滞留 profile tab → like/follow 找不到红心）。
  ③ XNRequestHooks.m:13 #import 过时文件名修复（PnsRequestHooks.h → XNRequestHooks.h，
     清 VPS 目录后不再 file not found）。
  继承 143c：cookie_dump + net_like extra_headers（签名实验遗留资产）。
"""
...
# 1. Clean remote build dir (v1.4.144: 必清——验证 XNRequestHooks.m:13 修复后不再依赖
#    9月残留的 PnsRequestHooks.h 兜底；构建炸弹修复的实锤验证)
print("[0] Clean remote build dir (验证构建炸弹修复)...")
