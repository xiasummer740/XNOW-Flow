#!/usr/bin/env python3
"""build-vps-dylib-90.py — VPS 交叉编译 xnower.dylib（v1.4.90）
v1.4.90 修复：主线程嵌套 dispatch_sync 自锁死锁（search/collect 打开搜索/主页时
外层 dispatch_sync(main) 内调内部又 dispatch_sync(main) 的方法 → 主线程死锁 → 设备离线）。
复用 v1.4.89 流程；密码从 .env.local 读。
"""
...
# 2. Fix XNWindowHelper.h（scene-based 取 keyWindow，同 vps-build-xnower.py）
print("[2] Fix XNWindowHelper.h...")
sftp = ssh.open_sftp()
sftp.put(os.path.join(PROJECT, '.tmp-fix_xn.py'), f"{REMOTE}/fix_xn.py")
sftp.close()
...
# 6. Download
print("[6] Download...")
local_dir = os.path.join(PROJECT, 'build-artifacts-ci', 'xnower-90')
