#!/usr/bin/env python3
"""一次性：把 v1.4.138 dylib 上传 VPS → vps-inject 注入 → 生成 TikTok_XNOW_v1.4.138_BH.ipa → 下载本地"""
...
LOCAL_DYLIB = os.path.join(PROJECT, 'build-artifacts-ci', 'xnower-138', 'xnower.dylib')
OUT = 'TikTok_XNOW_v1.4.138_BH.ipa'
...
# 1. 上传 dylib
print("[1] 上传 xnower.dylib → /root/xnow-build/xnower-138.dylib ...")
sftp = ssh.open_sftp()
sftp.put(LOCAL_DYLIB, '/root/xnow-build/xnower-138.dylib')
sftp.close()
print("  uploaded")

# 2. 找基础 IPA：无 43.7.0 原始包，用 static 里最新的已注入版本(137)作为 base 连续注入
print("[2] 定位基础 IPA ...")
out, _ = run("ls -t /opt/xnow-flow/static/TikTok_XNOW_v1.4.13[0-9]_BH.ipa 2>/dev/null | head -1")
...
# 3. 注入
print(f"[3] vps-inject: {base_ipa} + xnower-138.dylib → {OUT} ...")
run(f"cd /opt/xnow-flow && python3 vps-inject.py {base_ipa} /root/xnow-build/xnower-138.dylib /opt/xnow-flow/static/{OUT} 2>&1", 300)
