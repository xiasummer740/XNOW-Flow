#!/usr/bin/env python3
"""一次性：144（like/follow 导航 bug 修复批 + 构建炸弹修复）打包 TikTok_XNOW_v1.4.144_BH.ipa"""
...
LOCAL_DYLIB = os.path.join(PROJECT, 'build-artifacts-ci', 'xnower-144', 'xnower.dylib')
OUT = 'TikTok_XNOW_v1.4.144_BH.ipa'
...
print("[1] 上传 xnower.dylib → /root/xnow-build/xnower-144.dylib ...")
sftp = ssh.open_sftp()
sftp.put(LOCAL_DYLIB, '/root/xnow-build/xnower-144.dylib')
sftp.close()
print("  uploaded")
...
run(f"cd /opt/xnow-flow && python3 vps-inject.py {base_ipa} /root/xnow-build/xnower-144.dylib /opt/xnow-flow/static/{OUT} 2>&1", 300)
