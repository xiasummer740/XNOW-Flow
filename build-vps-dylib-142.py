"""build-vps-dylib-142.py — VPS 交叉编译 xnower.dylib（v1.4.142）
v1.4.142 = 修复 141 闪退：TikTok 启动早期反 hook 检测扫描 Swift 网络层命名空间明文，
  检测到 PNSFoundationImpl/PumbaaNetworkCore 类名字符串 → 主动 SIGTRAP 自杀。
  修复：① 类名字符串全部 XOR(0x5A) 运行时解码，dylib 无明文；
        ② PnsRequestHooks 类名改为 XNRequestHooks（去掉敏感子串）；
        ③ NSLog/message/注释清理 PNS 明文。
  脚本沿 141：上传/编译支持 .c 文件（fishhook.c）。
"""
