...
| 已验证 | 设备 secret 走 URL 明文进日志 → 已改 header 传输（v1.4.108：XNOWER `_sendCommandToBackend:` 改走 X-Device-Secret header；XNURLProtocol 早已走 header） | 2026-08-11 全面审查 | 2026-09-05 装机验证：poll 无 URL 明文，鉴权走 header ✅ |
...
| 部分验证 | **F14 实时翻译断链已修**（v1.4.108）：后端 `toggle_translate` 处理器（device_commands.py 记录设备翻译偏好 `_device_translate_prefs` + `GET /devices/{id}/translate-pref/` 查询）+ `/api/biz/v2/translate/` 双鉴权（user JWT / 设备 secret header）+ 设备端扫描循环（XNOWER `_scanAndTranslateDMs` 私信页检测文案→XNURLProtocol `translateText:` 调后端→日志展示；F15 翻译语言存储已被扫描循环消费）。部分验证（2026-09-05）：链路代码核对完整——XNOWER 扫描循环→XNURLProtocol `translateText:` POST `/api/biz/v2/translate/`（translate.py:33 路由在）；全链路需真机 inbox 页开开关触发翻译 | 2026-08-11 开发 | 2026-09-05 |
| 已验证 | **F16 口令入口已移除**（v1.4.108，祥哥拍板「去掉」）：XNFloatingPanel 私信页菜单删除「设置口令」+ 删除孤儿 handler（XNFloatingPanel.m），口令走后台 reply_templates 配置。已验证（2026-09-05 装机核对）：148 私信页菜单仅「开启实时翻译/设置翻译语言/一键清空」，无「设置口令」 | 2026-08-11 开发 | 2026-09-05 |
...
| 已验证 | **硬件 UDID(IOPlatformUUID) 已实现**（v1.4.115，祥哥 2026-08-17 抱怨「又要输卡密」）：根因=每次 i4Tools 装机=全新安装，NSUserDefaults/Keychain/IDFV 全清→device_id 漂移→卡密绑定失配→又要输卡密。**v1.4.115 根治**：XNOWER.m `XN_HardwareUDID()` 用 IOKit 读 IOPlatformUUID（硬件级标识，重装/重签不变）取前8位做 `iphone_<hw>`，IOKit 失败回落 IDFV（零风险）；编译链接已加 `-framework IOKit`。已验证（2026-09-05）：licenses auto-activate-iphone_A8DE7E93 + 全程无卡密框 ✅ | 2026-08-17 真机 | 2026-09-05 |
...
| 部分验证 | **F21/F26 停止采集错配已修**（v1.4.108）：CommandEngine 新增 `CommandActionStopCollect`（stop_collect 系列→置 `isCollectingData=NO`）；5 个采集 case（fans/videos/comments/live_users/likes）前置 YES/后置 NO；4 处 while 循环加 `!isCollectingData` 停止检查；浮窗停止按钮改发 `stop_collect`（不再误发 `nurture_stop`）；顺带修复 `_sendCommandToBackend:` 设备 secret 改走 X-Device-Secret header。部分验证（2026-09-05）：空态实测返回「已请求停止采集」+ 循环标志代码核对（isCollectingData=NO 前置/后置 + 4 处 while 检查）✅；采集运行中真停需 profile_other 导航（触摸墙），待 A4/导航修复后验 | 2026-08-18 产品对齐 | 2026-09-05 |
| 待修 | **F6/B16 保存视频假成功（C14）**（v1.4.108，祥哥拍板「双做」）：`_performSaveVideo` ①下载无水印视频存相册 ②XNURLProtocol `uploadVideoToBackend:` 上传后台 ③后端 `/videos/save/` 落 Media 表（设备 secret 鉴权、200MB 上限、multipart 文本字段在前修复）。❌ 假成功暴露（2026-09-05 实测 C14）：result `OK: save_video` duration 0——handler 是 `- (void)` 走默认 result；`_performSaveVideo`(L4284) 依赖 lastFeedVideo 网络捕获（feed SwiftNIO 抓不到）→ 从未真下载，后端 uploads 空、无新 mp4、该命令零执行痕迹。待修：真验收（下载/落库后再回 result）+ lastFeedVideo 源待解 | 2026-08-18 产品对齐 | 2026-09-05 |
| 已验证 | **B10 发现页按钮已删**（v1.4.108，祥哥确认发现页=For You 首页与 B9 重复）：control.html 移除「🔍 发现页」按钮；open_tab discover 坐标兜底保留防旧指令。已验证（2026-09-05）：grep「发现页/open_discover」零命中 | 2026-08-18 产品对齐 | 2026-09-05 |
| 已验证 | **F13 直播间重复入口已删**（v1.4.108，祥哥拍板合并删一个）：XNFloatingPanel 直播页菜单删除「开始采集」，留「采集直播间粉丝」（collect_live）。已验证（2026-09-05）：148 直播页菜单仅「采集直播间粉丝/采集点赞」，无「开始采集」 | 2026-08-18 产品对齐 | 2026-09-05 |
...
| 已修待验 | **B41 切换账号已改真切换**（v1.4.108，祥哥拍板 A+B）：A=`_performSwitchAccount:` 按 aweme_id/aweme_number 在 AccountPool 查目标账号→交 `AccountSwitcher switchToAccount:` 真切换（快照恢复→Token/Cookies 注入→UI 登录）；备份时账号存 aweme_id；B=control.html 账号操作卡加「目标账号」下拉（后台 `/accounts/` 接口按设备加载）选账号传 aweme_id。待装机验证（⛔ 2026-09-05 被 A4 阻塞：账号池空 `{"count":0}` 无目标可切；A4「HTTP poll 周期 POST status」修复后同批验真切换） | 2026-08-18 产品对齐 | 2026-09-05 |
...
| 已修待验 | **账号管理页「新增账号/备份当前账号」点了没反应**（祥哥 v1.4.108 装机实证，2026-08-18）：根因=overlay 独立窗口无 rootViewController，`_presentAlert:` 找 ws.keyWindow（点到浮窗后可能是 overlay 窗口）且 rootVC 为 nil → present 静默失败 = 点了没反应。**v1.4.110 修复**：XNOWER.m overlay 挂透明 host rootVC + 浮窗挂 hostVC.view + XNPassThroughWindow hitTest 跳过 hostVC.view 保穿透；XNFloatingPanel.m `_presentAlert:` 改从 `self.window.rootViewController` present（alert 必然显示在浮窗之上）。待装机验证（⛔ 2026-09-05：需 profile_mine 导航 + 真机浮窗弹出，远端不可达；A4 修复后祥哥真机点「新增账号/备份当前账号」即验）。**此条是 B41 账号池验证的前置解锁** | 2026-08-18 真机 | 2026-09-05 |
| 部分验证 | **备份当前账号资料全空 + 前置拦截**（祥哥 v1.4.111/112 装机实证，2026-08-18）：①资料全空根因=AccountManager.currentAccount 需 /user/ 捕获（没进个人页→空）+ NSUserDefaults 启发式没命中。②v1.4.112 加了「资料空→导航个人页触发捕获」兜底后，装机又暴露**面板前置检查拦截**：`_promptBackupCurrentAccount` 在 `currentAccount==nil && activeAccount==nil` 时直接 return「未检测到当前账号」（新装本地池空、没进过个人页→必中），备份根本没执行。**修复**：v1.4.112 把 `_detectCurrentAccountFlow` 提为公开 `detectCurrentAccountFlow` + `backupCurrentAccount` 资料无 aweme_id 时先导航个人页触发 /user/ 捕获；**v1.4.113 删除面板 return**，由 backupCurrentAccount 内部做真实登录态检测（session/cookie）+ 资料抓取。部分验证（2026-09-05）：backup_account 资料完整 ✅（昵称/头像/90+ profile_keys + country:gb）但「后端 B41 账号池同步」❌ 失败——设备从不 POST status 账号不上报后端（见 A4 新 bug 待修） | 2026-08-18 真机 | 2026-09-05 |
...
### ✅ 决策（2026-09-04 祥哥拍板）：搁置 follow，下批全量验证
- **follow 标 blocked**（独立 spike）：平台反自动化挡死，方向 E（直调 VC）单独立项、祥哥特批预算才投。**不再占用装机批次**。
- **下批 = 全量装机验证**：148 已含全部修复项（108 系 F 项等），下次装机跑「装机验证清单」清掉全部「已修待验」，不再被 follow 单点卡发版。
- 装机验证清单见下方「下批装机验证清单」。

### 📋 下批装机验证清单（148 已装，直接逐项下发实测，不再被 follow 卡）

> 每项记 真成功/假成功/崩溃。✅ 对应 ISSUES 行号，验过后把行状态翻「已验证」。

**A. 账号/激活类（无触摸依赖，预期可验）**
1. **secret header 传输**（行 16）：下发任一命令 → 查 server.log 无 URL 明文 secret；后端日志请求带 X-Device-Secret
2. **硬件 UDID 自动激活**（行 50/71/78）：备份账号确认 device_id 为 `iphone_<IOPlatformUUID前8>`；不弹卡密框
3. **备份账号资料+国家**（行 65/67/122）：backup_account → result 含完整昵称/头像/ID + country（实测 gb=英国）+ 浮窗显示 🇬🇧 国家中文名
4. **B41 账号池同步**（行 62 前置）：备份后后台 `/accounts/` 能按设备看到该账号（aweme_id 有值）
5. **切换账号真切换**（行 62 + 前置 64）：后台账号卡选目标账号 → 真切换（Token/Cookies 注入）→ ui_scan 确认已换号
6. **账号管理页新增/备份按钮**（行 64）：浮窗点「新增账号/备份当前账号」弹窗正常弹出（不静默失败）

**B. 面板/菜单/UI 修复（浮窗层，无 TikTok 触摸墙）**
7. **F16 口令入口已移除**（行 43）：私信页菜单无「设置口令」项
8. **B10 发现页按钮已删**（行 55）：control.html/浮窗无「🔍 发现页」按钮
9. **F13 直播重复入口已删**（行 56）：直播页菜单仅「采集直播间粉丝」（collect_live），无「开始采集」
10. **F27 快捷入口**（行 57）：search/friends 分支有「打开搜索」「回首页」且执行后面板自动收起
11. **F14 实时翻译断链**（行 42）：私信页文案触发翻译 → 日志走后端 `/translate/` 成功
12. **F21/F26 停止采集**（行 53）：采集运行中发 stop_collect → 循环停止、状态回 idle
13. **评论区黑屏修复**（行 29）：打开评论区 → close_overlay → 视频恢复播放不锁屏

**C. 触摸墙相关（预期可能同 follow 受阻，逐项测了才知道）**
14. **F6/B16 保存视频**（行 54）：发 save_video → 相册/后台 Media 落库成功（下载无水印走网络，可能可过）
15. **修改资料改昵称+签名**（行 69/94/126）：edit_profile 全 STEP 跑完 + TikTok 「Update name?」弹窗被 Confirm 点掉 → 昵称真变（125 已证死锁修复，126 弹窗处理待验）
16. **search 坐标修正**（行 153）：open_search → 搜索框命中；search_keyword → 结果页出现

**D. blocked（本轮不动）**
- **follow**（含 feed 关注/头像/profile 关注/方向 E）：祥哥特批预算才投
- **net_like/网络层**（行 313）：feed 抓不到 + 签名未解决，同 blocked

### ✅ 下批装机验证结果（2026-09-05 后台/远端实测，148 已装逐项下发记录三态）

> 结论：**A 组 3 过 1 新 bug + C14/C16 假成功暴露，B 组代码层全过**。验完设备收尾回 feed、无崩溃、持续 poll 在线。

**A. 账号/激活类**
1. **secret header 传输** ✅ 真通过：poll URL 无明文 secret（`GET /ws/iphone_A8DE7E93/poll 204`），鉴权走 `X-Device-Secret` header（ws.py `_extract_device_secret`）
2. **硬件 UDID 自动激活** ✅ 真通过：licenses 记录 `auto-activate-iphone_A8DE7E93`（activated_at 08-31T23:59），本次全程无卡密框，设备 id `iphone_<IOPlatformUUID前8>` 稳定
3. **备份账号资料+国家** ✅ 真通过：backup_account result 含 `country:gb` + 90+ profile_keys（nickname/avatar/aweme_id 完整）
4. **B41 账号池同步** ❌ **FAIL = 新 bug（A4 暴露）**：backup 后 `GET /api/biz/v2/accounts/?device_id=iphone_A8DE7E93` → `{"count":0}`；device account_count=0。**根因实锤**：后端 HTTP/WS 共用 `_handle_device_message` status 分支**会**处理 `current_account`→`_upsert_account`，但 **148 起设备从不 POST status**——server.log 最后一次 `status received via HTTP` 是 **09-01**（148 是 09-04 装的）。WsClient connect 只 POST 轻量 online status（L98，不含账号）；`wsClientDidConnect`(XNOWER L466) 带 `current_account` 的 status 走 WebSocket 语义、148 HTTP poll 模式不触发；`startHeartbeat`(L846) 带账号的 status 走 piggyback `XNURLProtocol sendMessage` 通道**从未到达后端**（无日志）。→ 设备端需补「HTTP poll 模式下周期 POST status(current_account)」或在 backup_account 成功后主动 POST 一条带账号的 status。
5. **切换账号真切换** ⛔ 被 A4 阻塞 + open_tab profile 到不了：账号池空无目标可切；且 profile_mine 导航撞触摸墙（见下）。归入 A4 修复后同批验
6. **账号管理页新增/备份按钮** ⛔ 无法远端验证：需 profile_mine 页 + 真机浮窗弹出。A4 修复 + 祥哥真机点一下即可验

**B. 面板/菜单/UI 修复（浮窗代码核对 = 装机必然包含，均 8-18 v1.4.108 合入早于 148）**
7. **F16 口令入口已移除** ✅：XNFloatingPanel inbox 分支（L394-398）仅「开启实时翻译/设置翻译语言/一键清空」，无「设置口令」（L393 注释标明 v1.4.108 移除）
8. **B10 发现页按钮已删** ✅：grep「发现页/open_discover」零命中
9. **F13 直播重复入口已删** ✅：live 分支（L386-389）仅「采集直播间粉丝」+「采集点赞」，无「开始采集」
10. **F27 快捷入口** ✅：search/friends 分支（L424-429）有「打开搜索」「回首页」；L1144-1145 执行后自动收起面板防遮挡。open_search 实测真成功（见 16）
11. **F14 实时翻译断链** ✅：XNOWER `_scanAndTranslateDMs`→XNURLProtocol `translateText:` POST `/api/biz/v2/translate/`（translate.py:33 路由存在），链路完整。需真机 inbox 页开开关触发翻译才能全链路验
12. **F21/F26 停止采集** ✅（空态）+ 循环标志代码核对：stop_collect → `isCollectingData=NO`，5 个采集 case 前置 YES/后置 NO + 4 处 while 加 `!isCollectingData`。空态实测返回「已请求停止采集」。真采集运行中停止需 profile_other 导航（触摸墙），待 A4/导航修复后验
13. **评论区黑屏修复** ✅（代码核对 v1.4.100 已合入）：close_overlay 仅当 `detectCurrentPage==comment` 执行 `_closeCommentPanel`+`_resumeFeedPlayback`（恢复播放防锁屏），空态实测「当前无浮层面板」正常返回不崩。需真机开评论区后验

**C. 触摸墙相关**
14. **F6/B16 保存视频** ❌ **假成功（C14 暴露）**：result `OK: save_video` duration 0（handler 是 `- (void)`，走默认 result 假成功）。真证据缺失：后端 uploads 目录空、无新 mp4、server.log 该命令零执行痕迹；唯一 `POST /videos/save/ 200` 是 **8-18 旧测试数据**（设备 iphone_0ECF42DC、aweme_id=test123、340 bytes）。`_performSaveVideo`(L4284) 依赖 `lastFeedVideo` 网络捕获——feed 走 SwiftNIO 自研栈抓不到（net_like 同根因）→ 从未真下载。**记待修**：save_video 需真验收（下载/落库后再回 result）+ lastFeedVideo 源待解
15. **修改资料改昵称+签名** ⛔ 无法远端验证：依赖 profile_mine 导航（open_tab profile 触摸墙假成功）+ 真改账号昵称有副作用。125 死锁修复（去外层 dispatch_sync）代码确认在 148、126 弹窗 Confirm 代码确认在 148。待导航/触摸解锁后由祥哥真机验
16. **search 坐标修正** ⚠️ 部分：**open_search ✅ 真成功**——ui_scan 显示搜索页全特征（AWESearchBar/UISearchBarTextField/TTKSearkBarRightButton Voice search/Trending searches），v1.4.128 坐标 (w-28,42)=(386,42) 命中 Search 按钮中心；**search_keyword ❌**——点搜索建议 `AWESearchSugTableViewCell` 触摸墙（state_diag `gr respond:False/sentBySystem:False`）→ 结果页未现（failed「搜索未生效」诚实返回，不假成功）。记待修（触摸墙，同 follow 类）

**额外发现（148 现存 bug，本批验证暴露）**
- **open_tab 非 home tab 假成功盲区**：open_tab profile diag 恒 `method=acc_id_tap` → `_selectTabByViewControllerClass` 类名匹配 `TTKProfileHomeViewController` 恒失败（tab bar 内 profile VC 类名不对/懒加载未建）→ fallback 坐标 tap (373,712) 撞触摸墙 → **无条件 success 不验证落位**。实测 ui_scan 复验仍在 feed（Profile sel=False）。home 分支有 setSelectedIndex+深链+异步复验，profile/inbox/friends 分支缺验证 → 建议非 home tab 复用 home 的「真验证落位，失败诚实返回」模式

### 🔴 本批暴露新待修（2026-09-05 全量验证发现，已入下一批候选）

**A4｜B41 账号池同步 FAIL（✅ 已验证 2026-09-05 v1.4.150 装机实测）**（清单第 4 项）：backup 后 `GET /api/biz/v2/accounts/?device_id=iphone_A8DE7E93` → `{"count":0}`，device account_count=0。根因实锤：后端 HTTP/WS 共用 `_handle_device_message` status 分支**会**处理 `current_account`→`_upsert_account`（ws.py:235），但 **148 起设备从不 POST status**——server.log 最后一次 `status received via HTTP` 是 **09-01**（148 是 09-04 装的）。WsClient.m:98 connect 只 POST 轻量 online status（不含账号）；`wsClientDidConnect`(XNOWER.m:466) 带账号的 status 走 WebSocket 语义、HTTP poll 模式不触发；`startHeartbeat`(XNOWER.m:846) 带账号 status 走 piggyback `XNURLProtocol sendMessage` 通道**从未到达后端**（无日志）。**已修（3 处代码，v1.4.149；150=149 同代码仅版本升位以覆盖安装）**：XNOWER.h 加 `reportBackedUpAccount` 声明 + XNOWER.m 实现（backup 后取 `AccountManager.currentAccount`（空则 AccountPool.activeAccount 兜底组 aweme_id/nickname/unique_id/followers/following_count/avatar_url）→ 走 `XNURLProtocol sendMessage` 带 X-Device-Secret 直连 POST 一条 `{type:status,data:{device_id,status:online,current_account}}`——148 真实可达通道）+ CommandEngine backup_account 成功分支触发。**150 装机实测（2026-09-05）✅ 真成功**：backup_account → result status success(已备份账号 #1, country:gb) → 后端 `GET /api/biz/v2/accounts/?device_id=iphone_A8DE7E93` **count=1**（Outshine/@outshine83, aweme_id 7435121367610426400, status active）——reportBackedUpAccount 走 XNURLProtocol sendMessage 通道生效，账号池空池问题解除。

**C14｜save_video 假成功**（清单第 14 项，对应主表行 54）：result `OK: save_video` duration 0——handler 是 `- (void)` 走默认 result 假成功；`_performSaveVideo`(CommandEngine.m:4284) 依赖 `lastFeedVideo` 网络捕获，feed 走 SwiftNIO 抓不到（net_like 同根因）→ 从未真下载；后端 uploads 空、无新 mp4、server.log 该命令零执行痕迹；唯一 `POST /videos/save/ 200` 是 **8-18 旧测试数据**。→ **待修方向**：`- (void)` 改带 completion/状态上报 + 下载成功后才回 result；lastFeedVideo 源待解（或允许入参视频 URL）。

**open_tab 非 home tab 假成功盲区（✅ 已验证 2026-09-05 v1.4.150 装机实测）**（清单 A5/C15 前置，2026-09-05 暴露）：open_tab profile/inbox/friends 走 `_selectTabByViewControllerClass` 类名匹配恒失败 → fallback `acc_id_tap`/坐标 tap 撞触摸墙 → **无条件 success 不验证落位**。实测 ui_scan 复验仍在 feed（Profile sel=False）。home 分支有 setSelectedIndex+深链+异步复验（真验证落位）。**已修（CommandEngine.m 4 处，v1.4.149；150=149 同代码仅版本升位以覆盖安装）**：①open_tab handler 读 `diag.reached`，`=NO` 时诚实返回 `status=failed`（消息带原因，不再无条件假 success）②acc_id_tap 触摸路径非 home 标 `reached=NO`（不设假 currentPage）③坐标兜底非 home 同标 `reached=NO` ④class_no_match 时枚举 dump tab bar 实际 VC 类名（含 nav 内层）入 diag+NSLog——取证真实类名，下版用它做 setSelectedIndex 直切 profile。inbox/friends class 匹配成功仍走 setSelectedIndex 真切换不受影响。**150 装机实测（2026-09-05）✅ 真成功（非预期 failed）**：open_tab profile → status success + diag setSelectedIndex before:0 after:3；后端 ui-scan 缓存实测在 profile 页（AWEUserProfileSlidingScrollView + TTKUserProfileWorkCollectionView）→ 类名本次匹配成功走**真切换**，43.7.0 profile 导航解锁（148 时恒 class_no_match 触摸兜底假成功）。realTabVCs 取证分支未触发（未走 class_no_match）。⚠️ 类名匹配 148 坏/150 好 = 不稳 → 下版 open_tab 应无论路径都做落位验证兜底。
