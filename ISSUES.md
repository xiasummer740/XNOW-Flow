...
**A4｜B41 账号池同步 FAIL**（清单第 4 项）：backup 后 `GET /api/biz/v2/accounts/?device_id=iphone_A8DE7E93` → `{"count":0}`，device account_count=0。根因实锤：后端 HTTP/WS 共用 `_handle_device_message` status 分支**会**处理 `current_account`→`_upsert_account`（ws.py:235），但 **148 起设备从不 POST status**——server.log 最后一次 `status received via HTTP` 是 **09-01**（148 是 09-04 装的）。WsClient.m:98 connect 只 POST 轻量 online status（不含账号）；`wsClientDidConnect`(XNOWER.m:466) 带账号的 status 走 WebSocket 语义、HTTP poll 模式不触发；`startHeartbeat`(XNOWER.m:846) 带账号 status 走 piggyback `XNURLProtocol sendMessage` 通道**从未到达后端**（无日志）。→ **待修方向**：HTTP poll 模式周期 POST `status(current_account)`，或 `backup_account` 成功后主动 POST 一条带账号的 status。
...
**open_tab 非 home tab 假成功盲区**（清单 A5/C15 前置，2026-09-05 暴露）：open_tab profile/inbox/friends 走 `_selectTabByViewControllerClass` 类名匹配恒失败 → fallback `acc_id_tap`/坐标 tap 撞触摸墙 → **无条件 success 不验证落位**。实测 ui_scan 复验仍在 feed（Profile sel=False）。home 分支有 setSelectedIndex+深链+异步复验（真验证落位）。→ **待修方向**：非 home tab 复用 home 的「真验证落位，失败诚实返回」模式（同 148 段 L520 建议）。此 bug 阻塞账号管理页/切换账号/修改资料的真机验收。
