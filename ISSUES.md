...
**A4｜B41 账号池同步 FAIL（✅ 已修待验 2026-09-05，编 v1.4.149 装机验证）**（清单第 4 项）：backup 后 `GET /api/biz/v2/accounts/?device_id=iphone_A8DE7E93` → `{"count":0}`，device account_count=0。根因实锤：后端 HTTP/WS 共用 `_handle_device_message` status 分支**会**处理 `current_account`→`_upsert_account`（ws.py:235），但 **148 起设备从不 POST status**——server.log 最后一次 `status received via HTTP` 是 **09-01**（148 是 09-04 装的）。WsClient.m:98 connect 只 POST 轻量 online status（不含账号）；`wsClientDidConnect`(XNOWER.m:466) 带账号的 status 走 WebSocket 语义、HTTP poll 模式不触发；`startHeartbeat`(XNOWER.m:846) 带账号 status 走 piggyback `XNURLProtocol sendMessage` 通道**从未到达后端**（无日志）。**已修（3 处代码，v1.4.149）**：XNOWER.h 加 `reportBackedUpAccount` 声明 + XNOWER.m 实现（backup 后取 `AccountManager.currentAccount`（空则 AccountPool.activeAccount 兜底组 aweme_id/nickname/unique_id/followers/following_count/avatar_url）→ 走 `XNURLProtocol sendMessage` 带 X-Device-Secret 直连 POST 一条 `{type:status,data:{device_id,status:online,current_account}}`——148 真实可达通道）+ CommandEngine backup_account 成功分支触发。待装机验：backup_account → 后端 `/api/biz/v2/accounts/` count=1。
...
**open_tab 非 home tab 假成功盲区（✅ 已修待验 2026-09-05，编 v1.4.149 装机验证）**（清单 A5/C15 前置，2026-09-05 暴露）：open_tab profile/inbox/friends 走 `_selectTabByViewControllerClass` 类名匹配恒失败 → fallback `acc_id_tap`/坐标 tap 撞触摸墙 → **无条件 success 不验证落位**。实测 ui_scan 复验仍在 feed（Profile sel=False）。home 分支有 setSelectedIndex+深链+异步复验（真验证落位）。**已修（CommandEngine.m 4 处，v1.4.149）**：①open_tab handler 读 `diag.reached`，`=NO` 时诚实返回 `status=failed`（消息带原因，不再无条件假 success）②acc_id_tap 触摸路径非 home 标 `reached=NO`（不设假 currentPage）③坐标兜底非 home 同标 `reached=NO` ④class_no_match 时枚举 dump tab bar 实际 VC 类名（含 nav 内层）入 diag+NSLog——取证真实类名，下版用它做 setSelectedIndex 直切 profile。inbox/friends class 匹配成功仍走 setSelectedIndex 真切换不受影响。待装机验：open_tab profile 诚实 failed + server.log 见 realTabVCs 真实类名。
