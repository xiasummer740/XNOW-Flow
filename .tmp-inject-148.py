#!/usr/bin/env python3
"""一次性：148（follow 方向 D：profile UIControl）打包 TikTok_XNOW_v1.4.148_BH.ipa"""
...
LOCAL_DYLIB = os.path.join(PROJECT, 'build-artifacts-ci', 'xnower-148', 'xnower.dylib')
OUT = 'TikTok_XNOW_v1.4.148_BH.ipa'
...
print("[1] 上传 xnower.dylib → /root/xnow-build/xnower-148.dylib ...")
sftp = ssh.open_sftp()
sftp.put(LOCAL_DYLIB, '/root/xnow-build/xnower-148.dylib')
sftp.close()
print("  uploaded")
...
run(f"cd /opt/xnow-flow && python3 vps-inject.py {base_ipa} /root/xnow-build/xnower-148.dylib /opt/xnow-flow/static/{OUT} 2>&1", 300)
