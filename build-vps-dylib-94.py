#!/usr/bin/env python3
"""build-vps-dylib-93.py — VPS 交叉编译 xnower.dylib（v1.4.93）
v1.4.94 修复里程碑（窗口基础回归）：
1. 删除编译期坏补丁：v1.4.89~92 的 build 脚本会在编译前上传执行 .tmp-fix_xn.py，
   把 XN_ActiveWindow 替换成"只认 isKeyWindow、不跳浮窗层(XNPassThroughWindow)、无 fallback"的坏版本，
   导致 ui_scan/页面识别/导航判断以浮窗为根、全部连锁失败。
   本次让源码即产物 — XNWindowHelper.h 源码（跳过 overlay + 多策略 fallback）直接生效。
2. 编译前守卫：grep 自检 VPS 上 XNWindowHelper.h 是好版本（含 XN_IS_OVERLAY、不含坏特征串），防静默覆盖。
复用 v1.4.92 流程；密码从 .env.local 读。
"""
...
# 2. Guard: 确认 VPS 上 XNWindowHelper.h 是好版本（编译前自检，替代旧 fix_xn.py 坏补丁）
#    好版本特征串：if (w.isKeyWindow && !w.hidden && !XN_IS_OVERLAY(w)) return w;  → XN_IS_OVERLAY 出现 1 次
#    坏版本特征串：if (w.isKeyWindow) return w;  → 'isKeyWindow) return w' 出现 0 次
print("[2] Guard XNWindowHelper.h (好版本自检)...")
# 好版本 XN_IS_OVERLAY 出现 6 次（#define 1 + 策略1/2/3 使用 4 + #undef 1）→ 只要 != '0' 即可
out, ec = run(f"grep -c 'XN_IS_OVERLAY' {REMOTE}/XNWindowHelper.h", 30)
ok_good = out.strip() != '0'
# 坏版本特征串：if (w.isKeyWindow) return w; （无 overlay 跳过）必须不在
...
# 6. Download
print("[6] Download...")
local_dir = os.path.join(PROJECT, 'build-artifacts-ci', 'xnower-94')
